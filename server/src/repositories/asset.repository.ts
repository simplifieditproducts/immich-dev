...
export class AssetRepository {
  ...
  createWithMetadata(asset: Insertable<AssetTable> & { id: string }, size: number, metadata?: Array<{ key: AssetMetadataKey; value: object }>) {
    let query = this.db;
  ...
  @GenerateSql({ params: [DummyValue.UUID, DummyValue.BUFFER] })
  async getUploadAssetIdByChecksum(ownerId: string, checksum: Buffer) {
    return this.db
      .selectFrom('asset')
